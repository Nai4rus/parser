...
class NovokuznetskSuParser implements ParserInterface
{
    use \app\components\helper\metallizzer\Cacheable;

    const USER_ID  = 2;
    const FEED_ID  = 2;
    const SITE_URL = 'https://novokuznetsk.su';

    protected static $posts = [];

    public static function run(): array
    {
        $url  = self::SITE_URL.'/news/city/archive';
        ...
        $page->filter('#news_by_tag a.article')->each(function ($item, $i) {
            if ($i >= 10) {
                return;
            }
    ...
    {
        $url = Url::encode($item->link()->getUri());
        ...
        $tz = new DateTimeZone('Asia/Novokuznetsk');
        $dt = DateTime::createFromFormat('d.m.Y, H:i', $item->filter('small')->first()->text(), $tz);

        $image = $page->filter('.page-image img');

        $post = new NewsPost(
            self::class,
            html_entity_decode($page->filter('h1[itemprop="name"]')->first()->text()),
            '~',
        ...
        self::$posts[] = (new Parser())->fill(
            $post,
            $page->filterXpath('//div[@id = "tfs"]/node()')
        );
    }
