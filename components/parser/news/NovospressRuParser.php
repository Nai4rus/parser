...
class NovospressRuParser extends AbstractBaseParser
{
    ...
    protected function parseNewsPage(PreviewNewsDTO $previewNewsDTO): NewsPost
    {
        ...
        $image = null;
        $mainImageCrawler = $newsPageCrawler->filterXPath('//div[contains(@class,"intro-img")]//img[1]');
        if ($this->crawlerHasNodes($mainImageCrawler)) {
            $image = $mainImageCrawler->attr('src');
            $this->removeDomNodes($newsPageCrawler, '//div[contains(@class,"intro-img")]');
        }
        ...
        $contentCrawler = $newsPageCrawler->filter('#fulltext');
    ...
    {
        $descriptionCrawler = $crawler->filterXPath('//div[contains(@class,"intro-main")]/p');

        if ($this->crawlerHasNodes($descriptionCrawler)) {
            $descriptionText = Text::trim($this->normalizeSpaces($descriptionCrawler->text()));

            if ($descriptionText) {
                $this->removeDomNodes($crawler, '//div[contains(@class,"intro-main")]/p');
